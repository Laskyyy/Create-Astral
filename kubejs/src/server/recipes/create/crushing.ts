export function createCrushingRecipes() {
  onEvent("recipes", (event) => {
    ...
    interface OreCrushingRecipe {
      mod: Special.Mod;
      ores: { name: string; extra?: Special.Item }[];
      suffix: string;
      crushed_prefix: string;
    }

    /// Raw
    const oreCrushingRecipes: OreCrushingRecipe[] = [
      {
        mod: "techreborn",
        ores: [
          {
    ...
    oreCrushingRecipes.forEach((mod) => {
      mod.ores.forEach((ore) => {
        ...
        let raw_ore: Special.Item = `${mod.mod}:raw_${ore.name}` as Special.Item;
        let raw_ore_block: Special.Item = `${mod.mod}:raw_${ore.name}_${mod.suffix}` as Special.Item;
        let crushed_ore: Special.Item = `${mod.crushed_prefix}:crushed_raw_${ore.name}` as Special.Item;
    ...
    interface OreDefinition {
      ore: keyof typeof oreMapping;
      name: Special.Item;
      block: Special.Item;
      bonus: number;
    }

    const ores: OreDefinition[] = [
      // Earth Ores
    ...
    ores.forEach((ore) => {
      let count: number = oreMapping[ore.ore].count + ore.bonus;
      let res: Special.Item = oreMapping[ore.ore].name;
      event.remove({
        type: "create:crushing",
        input: ore.name,
      });
    ...
    const lakyCrushingRecipes: CrushingRecipe[] = [
      {
        input: "create:ochrum",
        outputs: [
