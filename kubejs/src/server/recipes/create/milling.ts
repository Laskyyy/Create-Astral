export function createMillingRecipes() {
  onEvent("recipes", (event) => {
    ...
    // "(\d)*x (\w*:\w*)"
    // Item.of("$2", $1)
    interface MillingRecipe {
      input: Internal.ItemStackJS_;
      output: Internal.ItemStackJS_;
      chance: number;
    }

    const millingRecipes: MillingRecipe[] = [
      {
  ...
  function otherMillingRecipes(event: Internal.RecipeEventJS) {
    interface OtherMillingRecipe {
      input: Internal.ItemStackJS_;
      output: Internal.ItemStackJS_[];
    }

    const otherMillingRecipes: OtherMillingRecipe[] = [
      {
  ...
  function sifterRecipes(event: Internal.RecipeEventJS) {
    //? Old Sifter recipes that were transferred to the Millstone to update Create to 0.5.1

    interface SifterRecipe {
      inputs: { item: Special.Item }[];
      outputs: { item: Special.Item; chance: number }[];
      /** @deprecated Unused. */
      processingTime?: number;
    }

    const sifterRecipes: SifterRecipe[] = [
      {
