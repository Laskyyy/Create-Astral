export function entityMeltingRecipes() {
  onEvent("recipes", (event) => {
    const BLAZING_BLOOD_MADIC_AMOUNT = 1620;
    // const BLAZING_BLOOD_MADIC_AMOUNT = 4050;

    interface EntityMeltingRecipe {
      /** The entity to melt. */
      entity: Special.EntityType,
      result: {fluid: Special.Fluid, amount: number},
      /** The amout of damage done per hit. */
      damage: number
    }

    const entityMeltingRecipes: EntityMeltingRecipe[] = [
      {
