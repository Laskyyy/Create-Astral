import { BUCKET, INGOT, mB, NUGGET, SLIMEBALL } from "fluid-constants";

export function castingTableRecipes() {
  onEvent("recipes", (event) => {
    interface CastingTableRecipe {
      fluidInput: {
        fluid: Special.Fluid;
        amount: number;
      };
    ...
    const castingTableRecipes: CastingTableRecipe[] = [
      {
    ...
    castingTableRecipes.forEach((recipe) => {
      let tableRecipe: { type: Special.RecipeType_; [key: string]: unknown } = {
