import { BUCKET, GEM, GEM_BLOCK, INGOT, mB, NUGGET } from "fluid-constants";

export function meltingRecipes() {
  onEvent("recipes", (event) => {
    ...
    interface MeltingRecipe {
      ...
      temp: number;
      time: number;
      byproducts?: { fluid: Special.Fluid; amount: number }[];
    }
    ...
    const meltingRecipes: MeltingRecipe[] = [
      ...
      {
        fluidInput: { fluid: "tconstruct:seared_stone", amount: (BUCKET / 4) * 7 }, // prevents skips to get scorched stone due to seared casting basins being obtainable premoon
        input: { item: "tconstruct:scorched_basin" },
        temp: 500,
        time: 125,
      },
      {
        fluidInput: { fluid: "tconstruct:seared_stone", amount: (BUCKET / 4) * 7 }, // prevents skips to get scorched stone due to seared casting tables being obtainable premoon
        input: { item: "tconstruct:scorched_basin" },
        temp: 500,
        time: 125,
      },
    ...
    meltingRecipes.forEach((recipe) => {
      let meltingRecipe: { type: Special.RecipeType_; [key: string]: unknown } = {
