export function astralGeneratorsBoilerRecipes() {
  onEvent("recipes", (event) => {
    boilerSteamFromFuel(event);
    boilerSteamFromLava(event);
  });

  function boilerSteamFromFuel(event: Internal.RecipeEventJS) {
    event.custom({
  ...
  function boilerSteamFromLava(event: Internal.RecipeEventJS) {
    event.custom({
      type: "custommachinery:custom_machine",
