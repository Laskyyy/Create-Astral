export function interaction() {
  ...
  const conduitBlocks: Special.Block[] = [
  ...
  interface ProjectileConfig {
    ...
    particleType?: string;
    particleHasColor?: boolean;
    particleColorR?: number;
    particleColorG?: number;
    particleColorB?: number;
    explosionEnable: boolean;
    explosionStrength?: number;
    explosionDamageTerrain?: boolean;
  }

  const ammos: ProjectileConfig[] = [
    {
      ...
      particleHasColor: true, //? Type of particle and if its colour can be changed
      particleColorR: 0.31,
      particleColorG: 0,
      particleColorB: 0.7, //? Colour settings
    ...
    if (entity.type === "create:potato_projectile") {
      ammos.forEach((ammoType) => {
        if (entity.fullNBT.Item.id === ammoType.projectileItem) {
          server.scheduleInTicks(5, (event: any) => {
